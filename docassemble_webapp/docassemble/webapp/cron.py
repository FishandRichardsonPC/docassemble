...
import dateutil
from docassemble.webapp.backend import unpack_dictionary
...
from docassemble.webapp.server import UserModel, Role, db, set_request_active, save_user_dict, reset_user_dict, \
    release_lock, app, error_notification, user_interviews
from docassemble.webapp.users.models import UserDict, UserDictKeys, UserRoles
...
def get_filenames():
    ...
    for record in db.session.execute(
            select(UserDict.filename).where(UserDict.encrypted == False).group_by(UserDict.filename)):
        results.append(record.filename)
    return results


def get_records(filename, last_index):
    subq = select(UserDict.key, UserDict.filename, db.func.max(UserDict.indexno).label('indexno'),
                  db.func.count(UserDict.indexno).label('cnt')).group_by(UserDict.filename, UserDict.key).where(
        UserDict.filename == filename, UserDict.encrypted == False, UserDict.indexno > last_index).subquery()
    results = []
    for record in db.session.execute(
            select(UserDict.key, UserDict.filename, UserDict.dictionary, subq.c.indexno, subq.c.cnt).join(subq,
                                                                                                          and_(subq.c.indexno == UserDict.indexno)).order_by(
                    UserDict.indexno).limit(200)):
...
def delete_inactive_users():
    ...
    for item in db.session.execute(
            select(UserModel.id, UserModel.last_login).join(UserRoles, UserModel.id == UserRoles.user_id).where(
                    or_(*filters))).all():
    ...
    for user_id in candidates:
        last_interview = db.session.execute(
            select(UserDictKeys.user_id, db.func.max(UserDict.modtime).label('last_activity')).join(UserDict, and_(
                UserDictKeys.filename == UserDict.filename, UserDictKeys.key == UserDict.key)).where(
                UserDictKeys.user_id == user_id).group_by(UserDictKeys.user_id)).first()
        ...
        user_interviews(user_id=user_id, secret=None, exclude_invalid=False, action='delete_all',
                        delete_shared=delete_shared, admin=True)
...
def clear_old_interviews():
    ...
    for filename, days in days_by_filename.items():
        last_index = -1
        while True:
            subq = select(UserDict.key, UserDict.filename, db.func.max(UserDict.indexno).label('indexno')).where(
                UserDict.indexno > last_index, UserDict.filename == filename).group_by(UserDict.filename,
                                                                                       UserDict.key).subquery()
            results = db.session.execute(
                select(UserDict.indexno, UserDict.key, UserDict.filename, UserDict.modtime).join(subq, and_(
                    subq.c.indexno == UserDict.indexno)).order_by(UserDict.indexno).limit(1000))
            stale = []
            results_count = 0
    ...
    while True:
        subq = select(UserDict.key, UserDict.filename, db.func.max(UserDict.indexno).label('indexno')).where(
            UserDict.indexno > last_index, UserDict.filename.notin_(days_by_filename.keys())).group_by(
            UserDict.filename, UserDict.key).subquery()
        results = db.session.execute(
            select(UserDict.indexno, UserDict.key, UserDict.filename, UserDict.modtime).join(subq, and_(
                subq.c.indexno == UserDict.indexno)).order_by(UserDict.indexno).limit(1000))
        stale = []
...
def run_cron(the_cron_type):
    ...
    user_info = dict(is_anonymous=False, is_authenticated=True, email=cron_user.email, theid=cron_user_id,
                     the_user_id=cron_user_id, roles=[role.name for role in cron_user.roles],
                     firstname=cron_user.first_name, lastname=cron_user.last_name, nickname=cron_user.nickname,
                     country=cron_user.country, subdivisionfirst=cron_user.subdivisionfirst,
                     subdivisionsecond=cron_user.subdivisionsecond, subdivisionthird=cron_user.subdivisionthird,
                     organization=cron_user.organization, location=None, session_uid='cron', device_id='cron')
    base_url = docassemble.base.config.daconfig.get('url root',
                                                    'http://localhost') + docassemble.base.config.daconfig.get('root',
                                                                                                               '/')
            ...
            for filename in filenames:
                ...
                while True:
                    ...
                    for indexno, key, the_filename, dictionary, steps in records:
                        docassemble.base.functions.this_thread.current_info = dict(user=user_info, session=key,
                                                                                   secret=None,
                                                                                   yaml_filename=the_filename,
                                                                                   url=base_url, url_root=path_url,
                                                                                   encrypted=False, action=None,
                                                                                   interface='cron', arguments={})
                        last_index = indexno
                        try:
                        ...
                        for cron_type_to_use in cron_types:
                            ...
                            if re.search(r'_background$', cron_type_to_use):
                                docassemble.webapp.worker.background_action.delay(the_filename, user_info, key, None,
                                                                                  None, None,
                                                                                  {'action': cron_type_to_use,
                                                                                   'arguments': {}})
                            else:
                                try:
                                    docassemble.base.functions.reset_local_variables()
                                    ci = dict(user=user_info, session=key, secret=None, yaml_filename=the_filename,
                                              url=base_url, url_root=path_url, encrypted=False, action=cron_type_to_use,
                                              interface='cron', arguments={})
                                    ...
                                    if interview_status.question.question_type in ["restart", "exit", "logout",
                                        ...
                                        interview_status = docassemble.base.parse.InterviewStatus(
                                            current_info=dict(user=user_info, session=key, secret=None,
                                                              yaml_filename=the_filename, url=None, url_root=None,
                                                              encrypted=False, action=new_action['action'],
                                                              arguments=new_action['arguments'], interface='cron'))
                                        try:
                                            interview.assemble(the_dict, interview_status)
                                        except:
                                            pass
                                        save_status = docassemble.base.functions.this_thread.misc.get('save_status',
                                                                                                      'new')
                                        if save_status != 'ignore':
                                            save_user_dict(key, the_dict, the_filename, encrypt=False,
                                                           manual_user_id=cron_user_id, steps=steps,
                                                           max_indexno=indexno)
                                        release_lock(key, the_filename)
                                        interview_status.do_sleep()
                                        ...
                                        if interview_status.question.question_type == "response":
                                            if hasattr(interview_status.question, 'all_variables'):
                                                ...
                                                sys.stdout.write(docassemble.webapp.backend.dict_as_json(the_dict,
                                                                                                         include_internal=include_internal).encode(
                                                    'utf8') + "\n")
                                    ...
                                    release_lock(key, the_filename)
                                    sys.stderr.write("Cron error: " + str(key) + " " + str(the_filename) + " " + str(
